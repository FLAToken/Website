...
class ERC20_Token extends Contract
{
     ...
     * @throws \HttpClient\Exception\HttpClientException
     */
    public function tokenSold(): string
    {
        $result = $this->call("tokenSold");
        $tokenSold = $result[0] ?? null;
        if (!Validator::BcAmount($tokenSold)) {
            throw new ERC20Exception('Failed to retrieve total token sold amount');
        }

        $scale = $this->decimals();
        return bcdiv($tokenSold, bcpow("10", strval($scale), 0), $scale);
    }
